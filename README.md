...
```
  하지만 가끔씩 이상한 key값으로 넘겨주는 경우가 있으며, 이에 대해서는 더 연구가 필요하다.
...
- 마음 같아서는 for반복문을 사용해서 작성하고 싶었으나, 일단 기능구현을 목표로 두고 두번째 방식으로 작성하였습니다. 추후 프로젝트 발표 후에 개선하고 싶은 부분이다. 

### 3. 텍스트박스 확장 표현
- 기존에는 텍스트박스의 확장방향을 아래가 아닌 위쪽 방향으로 컨텐츠를 가리면서 확장하는 방식으로 하려고 여러가지 방법을 찾아보고 시도해보고 하였으나, 결국 기한내에 프로젝트를 마무리 하기 위해 차선책으로 아래쪽 방향으로 확장되도록 작성하였다.
  추후에 가능한 방법에 대해서 찾아보면서 공부해야한다.
...
```
- 페이지 전환 버튼은 위와 같이 작성하였다. 현재 프로젝트에서는 두개의 페이지 밖에 없으므로 이전 버튼을 클릭하면 다음 버튼을 나타나게 하고, 이전 버튼을 사라지게 하였다. 반대로 다음 버튼을 클릭하면 다음버튼을 사라지게 하고, 이전 버튼을 나타나게 하였다.
- 더 좋은 방식이 있을지 고민을 해야할 것 같다.
